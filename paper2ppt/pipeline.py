...
from paper2ppt.deck_builder import build_deck, ensure_title_slide, fill_missing_titles
    ...
    *,
    ...
    try:
        ...
        if len(slides) < raw_count:
            print(f"      Section merge: {raw_count} items -> {len(slides)} slides (section pages folded in)")
        if balanced_count > len(slides):
            print(f"      Length balance: {len(slides)} slides -> {balanced_count} slides (split overflow)")

        assigned = assign_figures_to_slides(slide_data["slides"], paper.figures)
        print(f"      Matched figures on {assigned} slide(s)")

        table_assigned = assign_tables_to_slides(slide_data["slides"], paper.tables)
        if paper.tables:
            print(f"      Matched tables on {table_assigned} slide(s)")
